...
void generate_initial_parameters(std::vector<REAL> & parameters_set, std::vector<Parameters> const & parameters)
{
    ...
    for (std::size_t i = 0; i < parameters_set.size() / n_parameters; i++)
    {
        ...
        parameters_set[1 + i * n_parameters] = parameters[i].peak_time * (a + b * uniform_dist(rng));
        parameters_set[2 + i * n_parameters] = parameters[i].rise_time * (a + b * uniform_dist(rng));
        parameters_set[3 + i * n_parameters] = parameters[i].decay_time * (a + b * uniform_dist(rng));
        parameters_set[4 + i * n_parameters] = parameters[i].baseline * (a + b * uniform_dist(rng));
    }
}
...
void generate_test_parameters(std::vector<Parameters> & target,    Parameters const source)
{
    ...
    for (std::size_t i = 0; i < n_fits; i++)
    {
        ...
        target[i].peak_time = source.peak_time;
        target[i].rise_time = source.rise_time;
        target[i].decay_time = source.decay_time;
        target[i].baseline = source.baseline * (a + b * uniform_dist(rng));
    ...
    std::vector<Parameters> const & parameters)
{
    ...
    for (std::size_t i = 0; i < n_fits; i++)
    {
        ...
        REAL const x0 = parameters[i].peak_time;
        REAL const rise = parameters[i].rise_time;
        REAL const decay = parameters[i].decay_time;
        REAL const baseline = parameters[i].baseline;

        std::size_t const fit_index = i * n_points;

        for (int ix = 0; ix < n_points; ix++)
        {
            std::size_t const point_index = ix;
...
int main(int argc, char * argv[])
{
    ...
    std::size_t const size_x = 500;
    std::size_t const n_points = size_x;
    ...
    true_parameters.peak_time = 30.;
    true_parameters.rise_time = 1.;
    true_parameters.decay_time = 100.;
    true_parameters.baseline = 10.;
